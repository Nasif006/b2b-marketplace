...
<head>
    ...
    <title>Add Product — Supplier Panel</title>
    ...
    <style>
        ...
        .top-bar {
            background:linear-gradient(135deg,#1c1008,#2d1a0e);
            padding:0 28px;
            height:56px;
            display:flex;
            align-items:center;
        ...
        .field-input {
            width:100%;background:#fafaf8;border:1px solid #e7e5e4;
            color:#1c1917;border-radius:8px;padding:9px 12px;
        ...
        .image-preview-box {
            width:100%;height:160px;background:#f5f5f4;border:2px dashed #e7e5e4;
        ...
        .image-preview-box .placeholder { color:#a8a29e;font-size:13px;text-align:center; }

        .btn-submit {
            width:100%;padding:11px;background:linear-gradient(135deg,var(--amber),#b45309);
            color:white;border:none;border-radius:9px;font-size:15px;font-weight:600;
...
<div class="page-wrap">
    ...
    <div class="page-sub">Fill in the details below to list a product on the marketplace</div>
    ...
    <div class="form-card">
        <form method="POST" action="/supplier/products">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    <label class="field-label">Minimum Order Qty (MOQ) *</label>
                    <input type="number" name="moq" class="field-input"
                        placeholder="e.g. 10"
                        value="{{ old('moq') }}" required>
                    <div class="field-hint">Buyers must order at least this quantity</div>
                </div>
...
function previewImage(url) {
    ...
    if (url && url.startsWith('http')) {
        box.innerHTML = '<img src="' + url + '" onerror="this.parentNode.innerHTML=\'<div class=placeholder>Invalid image URL</div>\'">';
    } else {
        box.innerHTML = '<div class="placeholder"><i class="bi bi-image" style="font-size:28px;display:block;margin-bottom:6px;"></i>Image preview will appear here</div>';
    }
}
</script>
</body>
